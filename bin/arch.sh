yay -S sqlite mariadb mariadb-libs postgresql postgresql-libs redis libxml2 zlib ncurses5-compat-libs gdbm nss openssl readline libffi xz yaml-cpp unzip make gcc wget curl fzf openssl libyaml ripgrep zsh sqlite mariadb libmariadbclient mariadb-clients postgresql postgresql-libs redis memcached imagemagick ffmpeg mupdf mupdf-tools poppler yarn libxml2 memcached && yay -S ffmpeg mupdf mupdf-tools wget curl gcc git vim ripgrep zsh fzf make  openssl cmake unzip nvidia-dkms mesa docker docker-compose docker-machine docker-buildx asdf-vm unixodbc sxhkd ripgrep fzf gyp bat github-cli microsoft-edge-beta neovim fd
